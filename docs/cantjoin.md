...
![无法刷出motd](pics/cantjoin/nomotd.png)
![未知的主机](pics/cantjoin/unknown%20host.png)
#### 解决方案
**A 检查IP是否正确**
...
如果玩家使用了特定版本的PCL2启动器，可能会出现无法正常搜索和连接部分服务器的问题，建议更换PCL2版本，或使用HMCL或者官方启动器。

## ② 登录失败 : 无效会话(请尝试重启游戏及启动器) 
#### 问题描述
能刷新出外面的motd，但加入服务器后显示“无效会话”。

![有motd界面](pics/cantjoin/havemotd.png)
![无效会话](pics/cantjoin/nomojanglogin.png)
#### 解决方案
**A 尝试重启客户端**
...
如果有可靠的消息来源证实Mojang验证服务被干扰，那么只能使用科学上网手段进入游戏了。
